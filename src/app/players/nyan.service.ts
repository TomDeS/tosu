// @TODO: change document.getelementbyid
...
export class NyanService {
    ...
    changeDataSets(i, indices, labels, yVals, datas, names, nrOfNames, myChart) {
        const nrOfSimulations = this.nrOfSimulations
        if (i <= nrOfSimulations) {
            ...
            if (i === nrOfSimulations) {
                document.getElementById('winner').innerHTML = "Winner: " + names[indices[0]];
                for (var t = 0; t < nrOfNames; t++) {
                    if (t < nrOfNames - 1) {
                        str += names[indices[t]] + ", ";
                    }
    ...
    createGraph(names, playerLength, ctx) {
        const nrOfNames = playerLength;
        ...
        this.subscription = interval(1).subscribe(
            val => {
                this.changeDataSets(i++, indices, labels, yVals, datas, names, nrOfNames, myChart);
                if (val === this.nrOfSimulations) {
